		...
		if ($password == $row[1])
		{
			...
			if ($_FILES)
			{
				...
				if ($name != '')
				{
					...
					move_uploaded_file($_FILES['filename']['tmp_name'], "/home/" . $db_database . "/public_html/" . $username . "/" . $name);
					...
					if (!mysql_num_rows($result))
					{
						...
						popen('python thumbs.py ' . $username . ' ' . $name . ' ' . $db_database, 'w'); 
